...
class price24418(Base):
    __tablename__ = 'price24418'
    id = Column(Integer, primary_key=True, autoincrement=True)
    Code = Column(Integer)
    price17424 = Column(Integer)
...
def load_csv_to_db(csv_file_path):
 ...
 #   df['ExcDat'] = pd.to_datetime(df['ExcDat'], errors='coerce')
    ...
    for start_row in range(0, df.shape[0], batch_size):
        ...
        session.bulk_insert_mappings(price24418, data_to_insert)
        session.commit()
...
csv_file_path = '/home/agmt92/django_projects/mysite/meds/csv/24417.csv'
